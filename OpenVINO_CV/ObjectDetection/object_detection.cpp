...
namespace space
{
	...
	void ObjectDetection::SetParameters(const OpenModelInferBase::Params &params, float confidence_threshold, const std::vector<std::string> labels)
	{
	...
	void ObjectDetection::PasingO1D1x1xNx7(InferenceEngine::IInferRequest::Ptr request)
	{
		...
		request->GetBlob(output_shared_layers[0].first.c_str(), data, 0);
	...
	void ObjectDetection::UpdateDebugShow()
	{
		...
		if (!debug_track)
		{
			...
			cv::createTrackbar("Scaling", debug_title.str(), &initValue, 150,
